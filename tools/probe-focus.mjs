...
const profile = path.join(os.tmpdir(), 'probe-profile-' + PORT);
...
const out=await send('Runtime.evaluate',{expression:`(function(){
  const c=window.__scene?.camera||document.querySelector('canvas')?.__c;
  return JSON.stringify({
    cam: c?{x:c.position.x,y:c.position.y,z:c.position.z}:null,
    mode: typeof mode!=='undefined'?mode:null,
    sel: selected?{name:selected.exhibit.name, center:selected.center, slot:selected.slot.id, face:selected.slot.face}:null,
